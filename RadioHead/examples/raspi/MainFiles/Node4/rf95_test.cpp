...
#define NOD1_ADDRESS 11
#define NODE2_ADDRESS 22
#define NODE3_ADDRESS 33
#define THIS_NODE_ADDRESS 44
...
int main(int argc, const char *argv[])
{
  ...
  node_status_map.insert(std::pair<int, bool>(NOD1_ADDRESS, false));
  ...
  while (!flag)
  {
    ...
    else if (state == 4) // recv
    {
      if (manager.recvfrom(buf, &buflen, &from))
      {
        ...
        {
          printf("Got an acknowledgement, but it's not for me!\n");
        }
        else
        {
    ...
    else if (state == 5) // send turn broadcast
    {
      ...
      if ((itr = node_status_map.find(NODE5_ADDRESS))->second == true)
      {
    ...
    else if (state == 12) // receive turn acknowledgement
    {
      if (manager.recvfrom(buf, &buflen, &from))
      {
        ...
        {
          printf("Recived node turn acknowledgement\n");
          rf95.waitAvailableTimeout(2000);
          state = 4;
          master_node = false;
        }
    ...
    {
      printf("I will rebroadcast my saved data\n");
      sleep(2);
      if (!two_nodes)
      {
      ...
      sleep(2);
      printf("I finished brodcasting my data\n");
      wait_timer = millis();
    }
